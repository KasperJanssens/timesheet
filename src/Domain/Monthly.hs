...
data SpecificMonth = SpecificMonth {y :: Year, m :: MonthOfYear} deriving (FromJSON, ToJSON, Generic, Ord, Eq, Show)

instance FromHttpApiData SpecificMonth where
  parseUrlPiece t =
    let split = trace ("text to parse is " ++ show t) $ Text.split (== ',') t in
      do
        (yAsText, mAsText) <- if length split == 2 then
                      Right (head split, split !! 1)
                  else
                    Left "not exactly two pieces"
